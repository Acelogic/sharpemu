...
Two other live initialization calls were audited but already had the smallest
correct no-service contract:
...
- `kvYEw2lBndk`, `sceGameLiveStreamingInitialize`, is called at loaded
  ...
  function `0x000013e0`, validates `0x4000` and can initialize a real service.
  SharpEmu's existing `0x80A00007` no-service result is deterministic and GTA
  handles it by disabling live streaming.
