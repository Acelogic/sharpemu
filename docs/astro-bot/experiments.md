...
| E218 | Does current `par274/main` preserve the corrected controller animation and title milestone? | Merge `2effc63` incorporates upstream through `a9a4f51`. Release and all four test groups pass; the visible run classifies the ordered controller/wordmark sequence, reaches exact title start, loads `worldmap`, and returns to the known red frame. | Upstreaming is regression-safe. Keep the merge and resume at the title producer/composition boundary. | `artifacts/astro-bot/runs/20260716-233311-e218-post-upstream-controller-direct/` |
