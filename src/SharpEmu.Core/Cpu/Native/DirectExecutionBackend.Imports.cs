...
public sealed partial class DirectExecutionBackend
{
	...
	private OrbisGen2Result DispatchKernelDynlibDlsym()
	{
		...
		var symbolNid = ComputePsNid(symbolName);
		...
		out ulong address)
	{
		...
		lock (_dynamicHleTrampolineGate)
		{
			...
			entries[importIndex] = new ImportStubEntry(
				0,
				symbolNid,
				export,
				IsLeafImport(symbolNid),
				IsNoBlockLeafImport(symbolNid),
				ShouldSuppressStrlenTrace(symbolNid),
				IsImportLoopGuardBoundary(symbolNid),
				StableHash64(symbolNid));
