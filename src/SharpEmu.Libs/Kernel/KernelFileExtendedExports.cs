...
public static partial class KernelMemoryCompatExports
{
    ...
    private static int KernelFcntlCore(CpuContext ctx)
    {
        ...
        // Mono queries the standard descriptors while bringing up its console.
        // Report stdin as read-only and stdout/stderr as write-only.
        if (command == F_GETFL && fd is >= 0 and <= 2)
        {
            var flags = fd == 0 ? 0 : 1;
